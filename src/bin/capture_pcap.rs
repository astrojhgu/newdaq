...
use newdaq::{DataFrame, MetaData, NCH, NCH_PER_PKT, NCORR, NPORT_PER_BD, PKT_LEN};
    ...
    let _ = std::thread::spawn(move || {
        ...
        loop {
            ...
            if last_meta_data.gcnt + 1 != frame_buf1.meta_data.gcnt {
                ...
                println!(
                    "{} packets dropped",
                    frame_buf1.meta_data.gcnt - last_meta_data.gcnt + 1
                );
            }

            if !((last_meta_data.fcnt == 819 && frame_buf1.meta_data.fcnt == 0)
                || last_meta_data.fcnt + 1 == frame_buf1.meta_data.fcnt)
            {
