...
use newdaq::{DataFrame, MetaData, NCH, NCH_PER_PKT, NCORR, PKT_LEN, NPORT_PER_BD, StorageMgr};
        ...
        loop{
            ...
            if frame_buf1.meta_data.fcnt == 0 && frame_buf1.meta_data.pcnt == 0 {
                ...
                if !dropped {
                    ...
                    let mut time_file=std::fs::OpenOptions::new().create(true).append(true).open(time_file_path).expect("File open failed");
                    ...
                    for i in 0..NCORR{
                        ...
                        let mut outfile=std::fs::OpenOptions::new().create(true).append(true).open(fpath).expect("File open failed");
                        //let mut outfile=std::fs::File::create(fpath).unwrap();
                        outfile.write(&disk_data[i*NCH*std::mem::size_of::<Complex<f32>>()..(i+1)*NCH*std::mem::size_of::<Complex<f32>>()]).unwrap();
                    }
